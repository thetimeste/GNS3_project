...
#crea il progetto
project = create_project(server, name="sdn_project", height=100, width=150, zoom=50)
...
gns3=Gns3Connector("http://0.0.0.0:3080")
project = Project(name="sdn_project", connector=gns3)
project.get()
project.open()
# Find the node by its name
for node in project.nodes:
    if node.name == 'matgo01-my-router2-mt':
        # Get the first available interface and assign an IP address
        interfaces = node.get
        if interfaces:
            interface = interfaces[0]  # Modify this logic to select the correct interface
            interface.set_ip('192.168.1.1', 24)  # Provide the IP address and subnet mask
            interface.update()  # Apply changes
